...
A Flask-based EQ controller app for the **Tanchjim Bunny DSP** USB-C IEMs written in python. It communicates with the onboard KTMicro DSP over raw USB HID, providing a cross-platform (mainly Linux) alternative to the official Windows- and Android-only applications.
...
Run the script as root, preserving your environment variables so Flask's hidden imports resolve correctly:
